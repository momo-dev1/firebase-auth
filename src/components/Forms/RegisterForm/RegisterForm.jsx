  ...
  const formik = useFormik({
    ...
    validationSchema: object({
      ...
      birthday: date()
        ...
        .min("1924-01-01", "your birthday date must be 1922-01-01 or more")
        .max("2024-01-01", "invalid birthday date"),
      password: string()
        .required("please enter your password")
        .min(8, "your password must be 8 characters or more")
        .matches(
          /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/,
          "Passwords must have at least 8 characters and contain at least two of following: upper case letters, lower case letters, numbers"
        ),
    ...
    onSubmit: (values, { setFieldError }) => {
      if (getStorage("users")) {
        const [isIterateUsername, isIterateEmail] = checkUser(
          values.username,
          values.email
        );
